...
- [`design.md`](./design.md) -- what Azrienoch does with Jost's own
  outlines, letterform by letterform: the reasoning behind every
  deliberate deviation from the source font.
- [`axes.md`](./axes.md) -- the four axes Azrienoch exposes, their
  ranges, and why `opsz`/`slnt` aren't among them.
...
build pipeline (which derives its letterforms from Jost, not from
hand-drawn corners; see `design.md`) and had grown into a separate,
much larger effort in its own right: a full variable-font authoring
tool, now developed as
