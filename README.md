...
Not yet enforced automatically: detecting a breaking change (a removed
glyph, a narrowed axis range, a metric/kerning change that could shift
an existing layout, a changed glyph outline) straight from the diff,
independent of the commit message. Given the font isn't finished yet,
codifying that now would be premature -- it's a natural next step once
the letterform work settles down and the font has real consumers to
protect.
