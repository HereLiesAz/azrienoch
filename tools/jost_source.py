...
license copied to `third_party/jost/OFL.txt`) is a real, professionally
drawn geometric sans. Per the project owner's direction, this module
takes its actual outlines as the starting point for Azrienoch's own
letterforms, rather than a from-scratch primitive construction
(attempted first, then abandoned -- see git history). Nothing here is
redrawn: `ufo_build.py` copies these outlines through as-is, and
Azrienoch's own modifications (Helvetica-inspired terminals/apertures,
axis behavior) are layered on top of this real data in later passes.
...
def extract(glyph_name: str, wght: int, wdth: int, grad: int = 0) -> tuple[list, float]:
    ...
    does -- Jost has nothing analogous to draw from, unlike a donor
    with its own native `GRAD` axis would. This approximates it instead
    by sampling Jost's own outline at a NEARBY `wght` (`GRAD_TO_WGHT`
