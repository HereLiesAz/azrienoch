"""Letter-pair kerning for Azrienoch, extracted from vendored Jost's
own GPOS pair-positioning table (SIL OFL 1.1) rather than hand-tuned:
getting several thousand pairs to look right by eye is its own
multi-week type-design task, and Jost already did that work.
...
Filtered down to pairs where BOTH sides are one of this project's own
glyphs (`params.CHARS` -- every script this project covers: Jost's own
...
- `a` is built from `d`'s own contours, not Jost's separately-drawn
  ...
  approximation from a different letterform.
