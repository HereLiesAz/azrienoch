...
punctuation, Latin-1 Supplement, Latin Extended-A, and Cyrillic (no
Greek: Jost has almost none of it -- 4 codepoints total, confirmed
directly against its own cmap -- and sourcing it from a second donor
was tried and dropped by direct decision; see git history). Every
...
and `serifs.py`'s per-letter-class foot rules extend to accented
...
def _current_version() -> tuple[int, int, int]:
    ...
    as-is, for a local dev build outside CI."""
...
# Character set lives in params.py, not here -- kerning.py (which this
# module imports) needs the same set without importing this module
# back (a cycle).
CHARS = params.CHARS
...
def build_master_ufo(wght: int, wdth: int, serf: int, grad: int = 0) -> Path:
    ...
    font.info.familyName = "Azrienoch"
    ...
    # finding "Azrienoch Wght400_Wdth100_Serf0" where "Azrienoch"/
    # "Regular" belonged.
    ...
    # `next_version.py` for how a release's own version is decided.
    major, minor, patch = _current_version()
    font.info.versionMajor, font.info.versionMinor = major, minor * 100 + patch
    font.info.openTypeNameVersion = f"Version {major}.{minor}.{patch}"
    ...
    path = SOURCES_DIR / f"Azrienoch-{params.style_name(wght, wdth, serf, grad)}.ufo"
