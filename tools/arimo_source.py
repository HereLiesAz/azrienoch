...
basis as this project's own use of Jost: a freely licensed font, used
and modified as its license explicitly permits.
...
Rescaled from Arimo's own metrics (UPM 2048, x-height 1082) to
Azrienoch's own (UPM 1000, x-height 470) via the x-height ratio: 's'
sits entirely within the x-height box in both fonts.
...
def extract(ch: str, wght: int, wdth: int, grad: int = 0):
    ...
    docstring) and rescaled into Azrienoch's coordinate space -- same
